<template>
  <AppLayout>
    <PageHeader title="Genders" actionText="Add Gender" :actionCallback="addGender" />
              ...
              <MenuItems class="absolute right-0 z-10 mt-2 w-32 origin-top-right rounded-md bg-white py-2 shadow-lg ring-1 ring-gray-900/5 focus:outline-none">
                <MenuItem v-slot="{ active }">
                  <a href="#" :class="[active ? 'bg-gray-50' : '', 'block px-3 py-1 text-sm leading-6 text-gray-900']"
                    >Edit<span class="sr-only">, {{ gender.name }}</span></a
                  >
                </MenuItem>
                <MenuItem v-slot="{ active }">
                  <a href="#" :class="[active ? 'bg-gray-50' : '', 'block px-3 py-1 text-sm leading-6 text-gray-900']"
                    >Delete<span class="sr-only">, {{ gender.name }}</span></a
                  >
                </MenuItem>
              </MenuItems>
            </transition>
...
const addGender = () => {
  console.warn('TODO implement addGender')
}
</script>